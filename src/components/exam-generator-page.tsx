...
export function ExamGeneratorPage() {
  ...
  /** Mobile: collapse settings while answering to focus on the question */
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNeedsRetry(false);
      // On mobile, collapse settings so the question is the focus
    ...
    <div className="min-h-screen w-full bg-background text-foreground">
      ...
      <div className="mx-auto w-full max-w-6xl px-3 py-3 sm:px-5 sm:py-6 lg:px-8 lg:py-8 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        <div className="grid w-full grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-[minmax(280px,360px)_1fr] lg:gap-8 lg:items-start">
          ...
          <aside
            className={cn(
              "flex flex-col rounded-2xl sm:rounded-3xl border border-border bg-secondary/60",
          ...
          >
            ...
            <div className="flex items-center justify-between gap-3 p-4 sm:p-5 lg:p-6 pb-3 sm:pb-4">
              ...
              <button
                type="button"
                className="lg:hidden flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-border bg-card text-muted-foreground hover:text-foreground touch-manipulation"
            ...
            {/* Collapsible body: always open on lg+, toggle on mobile */}
            <div
              className={cn(
                "flex flex-col gap-4 sm:gap-5 px-4 sm:px-5 lg:px-6 pb-4 sm:pb-6",
                settingsOpen ? "flex" : "hidden lg:flex",
              )}
            >
              <ApiKeyPanel
                ...
                <div className="rounded-2xl border border-amber-500/40 bg-amber-50/80 dark:bg-amber-950/30 p-3.5 sm:p-4 space-y-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-amber-900 dark:text-amber-100">
                        ...
                        {wrongQuestions.length !== 1 ? "s" : ""} saved
                      </p>
                      <p className="text-[11px] leading-relaxed text-amber-800/80 dark:text-amber-200/70 mt-0.5">
                        Answer each correctly (retry allowed). Then generate new questions.
                      </p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      type="button"
                      size="sm"
                      className="flex-1 h-10 sm:h-9 rounded-xl text-xs font-semibold touch-manipulation"
                      onClick={() => startReview()}
                    ...
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      className="h-10 sm:h-9 w-10 sm:w-9 rounded-xl text-xs touch-manipulation shrink-0 px-0"
              ...
              <div className="space-y-2">
                ...
                {!docMode ? (
                  <button
                    ...
                    className="flex w-full min-h-[48px] items-center justify-center gap-2 rounded-xl border-2 border-dashed border-border bg-card px-4 py-3 text-sm font-medium text-muted-foreground hover:border-primary/40 hover:text-primary disabled:opacity-60 touch-manipulation"
                  >
                    {docExtracting ? (
                      <>
                      ...
                      <>
                        ...
                        <span className="text-left">Upload PDF, DOCX, or TXT</span>
                      </>
                    )}
                  </button>
                ) : (
                  <div className="flex items-center gap-2 rounded-xl border border-primary/30 bg-primary/5 px-3 py-2.5">
                    ...
                    <button
                      ...
                      className="rounded-md p-2 text-muted-foreground hover:text-destructive touch-manipulation"
                ...
                <p className="text-[11px] leading-relaxed text-muted-foreground hidden sm:block">
                  Best with text-based PDFs, DOCX, or TXT. Scanned images not supported.
                </p>
              ...
              {!docMode && (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="h-11 sm:h-11 rounded-xl text-base sm:text-sm"
              ...
              <Button
                ...
                className="h-12 w-full rounded-2xl font-semibold touch-manipulation text-base sm:text-sm"
              >
                {mutation.isPending ? (
                  <>
                  ...
                  <>
                    <BookOpen className="mr-2 h-4 w-4" /> Answer all wrong first
                  </>
              ...
              {questions.length > 0 && !reviewMode && (
                <Button
                  type="button"
                  variant="outline"
                  className="h-11 w-full rounded-xl text-xs font-bold uppercase tracking-widest touch-manipulation"
                  onClick={() =>
              ...
              {questions.length > 0 && (
                <p className="text-center text-sm text-muted-foreground">
                  {reviewMode ? "Re-exam left" : "Score"}:{" "}
                  {reviewMode
                    ? `${questions.length} to go`
          ...
          <main
            className={cn(
              ...
              /* Extra bottom space on mobile for sticky nav */
              hasActiveExam && "pb-20 lg:pb-0",
            )}
          >
            {mutation.isError && (
                ...
                {wrongQuestions.length > 0 ? (
                  <>
                    <p className="text-sm text-amber-800 dark:text-amber-200 max-w-sm">
                      {wrongQuestions.length} wrong question
                      {wrongQuestions.length !== 1 ? "s" : ""} saved. Re-exam starts next — answer
                      each correctly (retry until correct).
                    </p>
                    <Button
                      type="button"
                      className="rounded-xl h-11 touch-manipulation w-full sm:w-auto"
                      onClick={() => startReview()}
                    >
                  ...
                  <p className="text-sm text-muted-foreground">
                    Perfect score — you can generate new questions.
                  </p>
                )}
              </div>
            )}
                  ...
                  {hasWrongPending ? (
                    <>
                      ...
                      <p className="text-sm max-w-sm">
                        {wrongQuestions.length} wrong question
                        {wrongQuestions.length !== 1 ? "s" : ""} left. Answer each correctly before
                        generating new ones.
                      </p>
                      <Button
                        type="button"
                        className="mt-2 rounded-xl h-11 touch-manipulation"
                        onClick={() => startReview()}
                      >
                    ...
                    <>
                      ...
                      {reviewComplete && (
                        <p className="text-xs text-emerald-700 dark:text-emerald-400 mt-1">
                          All previous mistakes cleared — ready for new questions.
                        </p>
                      )}
                    </>
                  )}
              ...
              <>
                {reviewMode && (
                  <div className="rounded-xl border border-amber-500/30 bg-amber-50/60 dark:bg-amber-950/20 px-3 sm:px-4 py-2.5 text-xs sm:text-sm text-amber-900 dark:text-amber-100 leading-relaxed">
                    <span className="font-semibold">Re-exam</span> — answer correctly to pass. If
                    wrong, try the same question again until correct.
                  </div>
                )}
                ...
                <div className="flex items-center gap-3 px-0.5">
                  <p className="text-xs sm:text-sm font-medium text-muted-foreground whitespace-nowrap">
                    {reviewMode ? "Re-exam" : "Q"} {currentIndex + 1}/{questions.length}
                  </p>
                ...
                <div className="rounded-2xl border border-border bg-card p-4 sm:p-5 md:p-6 shadow-sm">
                  <p className="mb-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                    ...
                    {reviewMode && (
                      <span className="ml-2 text-amber-600">· must answer correctly</span>
                    )}
                  </p>
                    ...
                    {currentQuestion.options.map((opt, i) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "flex w-full items-start gap-3 rounded-xl border px-3.5 sm:px-4 py-3.5 sm:py-3 text-left transition-colors touch-manipulation",
                            "min-h-[52px] sm:min-h-0",
                            "border-border hover:border-foreground active:scale-[0.99]",
                        ...
                        >
                          <span
                            className={cn(
                              "flex h-8 w-8 sm:h-7 sm:w-7 shrink-0 items-center justify-center rounded-full border text-xs font-bold",
                              isRevealed &&
                                isAnswer &&
                          ...
                          <span className="flex-1 pt-0.5 text-sm sm:text-sm leading-snug">
                            {opt}
                          </span>
                          {isRevealed && isAnswer && (
                  ...
                  {revealed[currentQuestion.question_number] && (
                    <div
                      className={cn(
                        "mt-4 rounded-xl border-l-4 bg-secondary/60 p-3.5 sm:p-4",
                    ...
                    >
                      ...
                      {reviewMode && needsRetry && (
                        <div className="mt-3 space-y-2">
                          <p className="text-sm text-muted-foreground">
                            You must get this right before moving on. Read the explanation, then
                            try again.
                          </p>
                          <p className="text-sm leading-relaxed border-t border-border/60 pt-2">
                            {currentQuestion.explanation}
                          </p>
                          <Button
                            type="button"
                            className="mt-2 h-11 rounded-xl w-full sm:w-auto touch-manipulation"
                ...
                >
                  <div className="mx-auto max-w-6xl flex items-center gap-2">
                    <Button
                      type="button"
                      variant="outline"
                      className="h-12 flex-1 rounded-xl touch-manipulation text-sm"
                      disabled={isFirst || (reviewMode && needsRetry)}
                      onClick={() => setCurrentIndex((i) => Math.max(0, i - 1))}
                    >
                    ...
                    <span className="text-xs text-muted-foreground tabular-nums px-2 shrink-0">
                      {currentIndex + 1}/{questions.length}
                    </span>
                    <Button
                      type="button"
