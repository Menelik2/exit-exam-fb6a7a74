...
export function ExamGeneratorPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-50 via-slate-100/80 to-indigo-50/40 text-slate-900">
      <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/80 text-slate-900 shadow-sm backdrop-blur-xl supports-[backdrop-filter]:bg-white/70">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-3 py-3 sm:px-5 lg:px-8 lg:py-3.5">
          <div className="flex min-w-0 items-center gap-2.5">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-indigo-600 text-white shadow-md shadow-indigo-600/25">
              <ListChecks className="h-4 w-4" />
            </span>
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold tracking-tight text-slate-900 sm:text-base lg:text-[17px]">
          ...
          <div className="flex shrink-0 items-center gap-2">
            {hasActiveExam && (
              <div className="hidden items-center gap-2 sm:flex">
                ...
                <span className="hidden rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-700 lg:inline">
                  Score {correctCount}/{questions.length}
                </span>
              </div>
            )}
            <button type="button" className="inline-flex h-9 items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700 shadow-sm hover:border-slate-300 hover:bg-slate-50" onClick={() => setSettingsOpen((o) => !o)}>
      ...
      <div className="mx-auto w-full max-w-7xl px-3 py-4 sm:px-5 sm:py-6 lg:px-8 lg:py-8">
        {settingsOpen && (
          <section className="mb-4 overflow-hidden rounded-2xl border border-slate-200/80 bg-white/90 shadow-sm shadow-slate-200/50 ring-1 ring-slate-900/5 lg:mb-6">
            <div className="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-indigo-50/40 px-4 py-3 lg:px-6 lg:py-3.5">
              <h2 className="text-sm font-semibold tracking-tight text-slate-800 lg:text-base">Quiz setup</h2>
            </div>
            <div className="grid gap-4 p-4 lg:grid-cols-2 lg:gap-8 lg:p-6">
                ...
                {hasWrongPending && (
                  <div className="rounded-xl border border-amber-200 bg-amber-50 p-3">
                    ...
                    <div className="mt-2 flex gap-2">
                      <Button type="button" size="sm" className="h-8 rounded-lg bg-indigo-600 text-xs hover:bg-indigo-700" onClick={() => startReview()} disabled={reviewMode && reviewQuestions.length > 0}>
                        <BookOpen className="mr-1 h-3.5 w-3.5" />{reviewMode ? "Reviewing…" : "Start re-attempt"}
                      </Button>
                      <Button type="button" size="sm" variant="outline" className="h-8 rounded-lg text-xs" onClick={clearAllWrong}><Trash2 className="mr-1 h-3.5 w-3.5" /> Clear</Button>
                    </div>
                  </div>
                )}
              </div>
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-slate-500">Source document (optional)</Label>
                  ...
                  {!docMode ? (
                    <button type="button" onClick={() => document.getElementById("doc-upload")?.click()} disabled={docExtracting} className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 py-3 text-sm text-slate-500 hover:border-indigo-400 hover:text-indigo-600">
                    ...
                    <div className="flex items-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-3 py-2">
                      ...
                      <div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold">{docName}</p><p className="text-[10px] text-slate-500">{docText.length.toLocaleString()} chars</p></div>
                      <button type="button" onClick={() => { setDocText(""); setDocName(""); setDocError(null); }} className="p-1 text-slate-500 hover:text-red-600" aria-label="Remove"><X className="h-4 w-4" /></button>
                    </div>
                  )}
                ...
                {!docMode && (
                  <div className="space-y-1.5">
                    <Label htmlFor="topic" className="text-xs font-semibold text-slate-500">Topic / course</Label>
                    <Input id="topic" placeholder="e.g. Data Structures" value={topic} onChange={(e) => setTopic(e.target.value)} className="h-10 rounded-xl border-slate-200 bg-white lg:h-11" disabled={hasWrongPending} />
                  </div>
                )}
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Select value={difficulty} onValueChange={(v) => setDifficulty(v as Difficulty)} disabled={hasWrongPending}>
                      <SelectTrigger className="h-10 rounded-xl border-slate-200 bg-white lg:h-11"><SelectValue /></SelectTrigger>
                      <SelectContent>
                ...
                <label className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5">
                  <span className="flex items-center gap-2 text-sm"><Sparkles className="h-3.5 w-3.5 text-indigo-600" /> Auto-generate</span>
                  <Switch checked={autoGenerate} onCheckedChange={setAutoGenerate} disabled={hasWrongPending} />
                </label>
                <Button type="button" onClick={run} disabled={mutation.isPending || docExtracting || (!docMode && !topic.trim()) || !canGenerateNew} className="h-10 w-full rounded-xl bg-indigo-600 font-semibold shadow-md shadow-indigo-600/20 hover:bg-indigo-700 lg:h-11 lg:text-[15px]">
                ...
                {questions.length > 0 && !reviewMode && (
                  <Button type="button" variant="outline" className="h-9 w-full rounded-xl border-slate-200 text-xs" onClick={() => downloadExamPdf(questions, { title: quizTitle, difficulty })}>
        ...
        {mutation.isError && (
          <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{(mutation.error as Error).message}</div>
        )}

        {mutation.isPending && (
          <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-slate-200/80 bg-white/90 py-20 text-slate-500 shadow-sm ring-1 ring-slate-900/5">
        ...
        {showFinishSummary && (
          <div className="rounded-2xl border border-slate-200/80 bg-white/95 p-8 text-center shadow-sm shadow-slate-200/50 ring-1 ring-slate-900/5 lg:mx-auto lg:max-w-2xl lg:p-12">
            <CheckCircle2 className="mx-auto h-12 w-12 text-indigo-600" />
            <h2 className="mt-3 text-xl font-semibold tracking-tight">Attempt finished</h2>
            <p className="mt-1 text-sm text-slate-500">Score: <strong className="text-slate-900">{correctCount}/{questions.length}</strong> · {Math.round((correctCount / Math.max(questions.length, 1)) * 100)}%</p>
            {wrongQuestions.length > 0 ? (
              <div className="mt-4 space-y-2">
                <p className="text-sm text-amber-800">{wrongQuestions.length} incorrect — re-attempt available</p>
                <Button type="button" className="rounded-xl bg-indigo-600 hover:bg-indigo-700" onClick={() => startReview()}><RotateCcw className="mr-2 h-4 w-4" /> Re-attempt now</Button>
              </div>
            ) : (
              <p className="mt-3 text-sm text-slate-500">Perfect score. Generate a new quiz from Setup.</p>
            )}
          </div>
        )}
          ...
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white/70 py-16 text-center shadow-sm">
            {hasWrongPending ? (
              <>
                ...
                <p className="mt-3 text-sm text-slate-500">{wrongQuestions.length} question(s) left to review.</p>
                <Button type="button" className="mt-4 rounded-xl bg-indigo-600 hover:bg-indigo-700" onClick={() => startReview()}>Start re-attempt</Button>
              </>
            ) : (
              <>
                <ListChecks className="mx-auto h-8 w-8 text-indigo-600" />
                <p className="mt-3 text-sm font-medium text-slate-800">No active attempt</p>
                <p className="mt-1 text-xs text-slate-500">Open Setup, choose a topic, then click Start attempt.</p>
              </>
            )}
          </div>
        )}
        ...
        {hasActiveExam && currentQuestion && (
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-7 xl:grid-cols-[minmax(0,1fr)_320px] xl:gap-8">
            <div className="min-w-0 space-y-3 pb-24 lg:pb-0">
              ...
              <article className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm shadow-slate-200/40 ring-1 ring-slate-900/5">
                ...
                <div className="p-4 sm:p-5 lg:p-7 xl:p-8">
                  <p className="text-[15px] leading-relaxed text-slate-900 sm:text-base lg:text-lg lg:leading-relaxed">{currentQuestion.question}</p>
                    ...
                    {currentQuestion.options.map((opt, i) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "flex w-full items-start gap-3 rounded-xl border px-3.5 py-3.5 text-left text-sm transition-all lg:gap-4 lg:px-4 lg:py-4 lg:text-[15px] hover:shadow-md hover:shadow-slate-200/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/40",
                            !isRevealed && "border-slate-200 hover:border-indigo-400 hover:bg-indigo-50/60",
                            ...
                            isRevealed && !isSelected && !isAnswer && "border-slate-100 opacity-70",
                          )}
                        >
                          <span className={cn(
                            "mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 text-[9px] lg:h-6 lg:w-6",
                            isSelected && !isRevealed && "border-indigo-500 bg-indigo-600 text-white",
                  ...
                  {revealed[currentQuestion.question_number] && (
                    <div className={cn(
                      "mt-4 rounded-xl border-l-4 px-3.5 py-3.5 text-sm lg:mt-5 lg:px-4 lg:py-4",
                      ...
                      {reviewMode && needsRetry && (
                        <div className="mt-2 space-y-2">
                          ...
                          <Button type="button" size="sm" className="h-9 rounded-lg bg-indigo-600 hover:bg-indigo-700" onClick={retryCurrentQuestion}>
            ...
            <aside className="lg:sticky lg:top-[4.5rem] lg:self-start">
              <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white/95 shadow-sm shadow-slate-200/40 ring-1 ring-slate-900/5">
                <button type="button" className="flex w-full items-center justify-between border-b border-slate-100 bg-gradient-to-r from-slate-50 to-indigo-50/30 px-3 py-3 text-left text-sm font-semibold text-indigo-600 lg:cursor-default" onClick={() => setNavOpen((o) => !o)}>
                  Quiz navigation
                  <span className="text-xs font-normal text-slate-500 lg:hidden">{navOpen ? "Hide" : "Show"}</span>
                </button>
                <div className={cn("p-3 lg:p-4", navOpen ? "block" : "hidden lg:block")}>
                    ...
                    {questions.map((q, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "relative flex h-9 items-center justify-center rounded-lg border text-xs font-semibold transition-all hover:scale-[1.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/40 lg:h-10 lg:text-sm",
                            st === "current" && "border-indigo-500 bg-indigo-600 text-white ring-2 ring-indigo-500/30",
                            st === "answered" && "border-indigo-400 bg-indigo-50 text-indigo-700",
                            ...
                            st === "todo" && "border-slate-200 bg-white hover:border-indigo-400",
                          )}
                        >
                          {idx + 1}
                  ...
                  <ul className="mt-3 space-y-1.5 border-t border-slate-100 pt-3 text-[10px] text-slate-500 lg:text-[11px]">
                    <li className="flex items-center gap-1.5"><span className="h-3 w-3 rounded border border-indigo-500 bg-indigo-600" /> Current</li>
                    <li className="flex items-center gap-1.5"><span className="h-3 w-3 rounded border border-indigo-400 bg-indigo-50" /> Answered</li>
                    <li className="flex items-center gap-1.5"><span className="h-3 w-3 rounded border border-slate-200 bg-white" /> Not yet answered</li>
            ...
            <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 px-3 pb-[max(0.6rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur lg:hidden">
              <div className="mx-auto flex max-w-7xl gap-2">
                <Button type="button" variant="outline" className="h-11 flex-1 rounded-xl border-slate-200 text-sm" disabled={isFirst || (reviewMode && needsRetry)} onClick={() => setCurrentIndex((i) => Math.max(0, i - 1))}>
                  <ChevronLeft className="mr-1 h-4 w-4" /> Prev
                </Button>
                <Button type="button" variant="outline" className="h-11 flex-1 rounded-xl border-slate-200 text-sm" disabled={isLast || (reviewMode && needsRetry)} onClick={() => setCurrentIndex((i) => Math.min(questions.length - 1, i + 1))}>
