...
export function ApiKeyPanel({
  ...
  const onSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem(PROVIDER_LS_KEY, "gemini");
      setSaved(key);
      setApiKey("");
      setEditing(false);
      onKeyChange?.(key, "gemini");
    } catch (e) {
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-border bg-card/70 p-4">
      ...
      <p className="text-[11px] text-muted-foreground">
        Server key is used by default. Optional: paste your own key below to override.
      </p>
      ...
      {saved && !editing ? (
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-semibold text-foreground">Gemini connected</span>
      ...
      {showForm && (
        <form onSubmit={onSave} className="space-y-2">
          ...
          <div className="flex gap-2 pt-1">
            <Button
              type="submit"
              disabled={checking || apiKey.trim().length < 10}
